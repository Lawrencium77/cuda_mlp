...
SingleLayerPerceptron::SingleLayerPerceptron(const int dim_out,
                                             const int dim_in,
                                             const bool use_activation)
    : dim_out(dim_out), dim_in(dim_in), weights(dim_in, dim_out),
      grads(dim_in, dim_out), use_activation(use_activation) {}
...
Matrix &SingleLayerPerceptron::forward(Matrix &input) {
  ...
  // Using a ptr means we need to make sure that the input matrix is not
  // modified between the forward and backward call.
  inputs = &input;
