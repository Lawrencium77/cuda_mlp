...
int readBigEndianInt(std::ifstream &file) {
  ...
  return (int)((bytes[0] << 24) | (bytes[1] << 16) | (bytes[2] << 8) |
               bytes[3]);
}
...
read_mnist_images(const std::string &filename) {
  ...
  std::vector<std::vector<unsigned char>> images(
      number_of_images, std::vector<unsigned char>(n_rows * n_cols));

  for (int i = 0; i < number_of_images; ++i) {
    if (!file.read(reinterpret_cast<char *>(images[i].data()),
                   n_rows * n_cols)) {
...
void printImageArray(const std::vector<std::vector<unsigned char>> &images,
