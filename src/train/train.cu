...
void shuffle_dataset(std::vector<std::vector<unsigned char>> &images,
...
get_val_stats(MLP &mlp,
              const std::vector<std::vector<unsigned char>> &val_images,
              const std::vector<unsigned char> &val_labels, const int bsz,
              const int feat_dim) {
  ...
  return std::make_pair(total_loss / num_samples,
                        100 * total_correct / num_samples);
}

std::vector<std::vector<float>>
train_loop(MLP &mlp, std::vector<std::vector<unsigned char>> &train_images,
           std::vector<unsigned char> &train_labels,
           const std::vector<std::vector<unsigned char>> &val_images,
           const std::vector<unsigned char> &val_labels, const int num_epochs,
           const int bsz, const int feat_dim, const float lr,
           const bool verbose, const bool time_epoch) {
  ...
  for (int epoch = 0; epoch < num_epochs; ++epoch) {
    ...
    for (int batch_idx = 0; batch_idx < num_batches_per_epoch; ++batch_idx) {
      ...
      float loss =
          matsum(get_ce_loss(output, data_and_labels.second)) / current_bsz;
    ...
    std::pair<float, float> val_loss_and_acc =
        get_val_stats(mlp, val_images, val_labels, bsz, feat_dim);
    ...
    std::cout << "Validation Loss after epoch " << epoch + 1 << ": "
              << val_loss_and_acc.first << std::endl;
    std::cout << "Validation Acc after epoch " << epoch + 1 << ": "
              << val_loss_and_acc.second << "%" << std::endl;
  }

  std::vector<std::vector<float>> metrics = {train_losses, val_losses,
                                             val_accs};
  return metrics;
}
...
void train(const std::string config_file) {
  ...
  std::vector<std::vector<unsigned char>> train_images =
      read_mnist_images(train_image_file);
  std::vector<std::vector<unsigned char>> val_images =
      read_mnist_images(val_image_file);
  ...
  std::vector<std::vector<float>> metrics = train_loop(
      mlp, train_images, train_labels, val_images, val_labels,
      std::stoi(config["num_epochs"]), std::stoi(config["bsz"]),
      std::stoi(config["feat_dim"]), std::stof(config["learning_rate"]),
      std::stoi(config["verbose"]), std::stoi(config["time_epoch"]));
