...
void testPrintOp(Matrix &input1, Matrix &input2, float *data, int bsz,
                 int feats) {
...
void testAddConst(Matrix &input1, float *data, float value, int bsz,
                  int feats) {
  Matrix output = input1 + value;
  output.printData("Testing Add Const");
}

void testMulConst(Matrix &input1, float *data, float value, int bsz,
                  int feats) {
  Matrix output = input1 * value;
...
void testHadamard(Matrix &input1, Matrix &input2, float *data, int bsz,
                  int feats) {
  Matrix output = input1 * input2;
...
void testAccuracy(Matrix &input, int num_classes, int bsz) {
  ...
  std::pair<Matrix, Matrix> loss_and_preds =
      get_ce_loss_and_accuracy(input, labels);
